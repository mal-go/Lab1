...
public:
	...
	void DrawPoly(const Vector2& pos, int sides, float radius, float rot, Color color) {
		DrawPolyLines(pos, sides, radius, rot, color);
	}

	void DrawUfo(const Vector2& pos, int sides, float radius, float rot, Color color) {
		DrawPolyLines(pos, 3, radius, rot, WHITE);
		DrawPolyLines(pos, 3, radius, rot+90, WHITE);
		DrawPolyLines(pos, 3, radius, rot+180, WHITE);
		DrawPolyLines(pos, 3, radius, rot+270, WHITE);
		DrawCircleV(pos, radius/2, color);
	}
...
public:
	TriangleAsteroid(int w, int h) : Asteroid(w, h) { baseDamage = 5; bonus = 5; }
	void Draw() const override {
		Renderer::Instance().DrawPoly(transform.position, 3, GetRadius(), transform.rotation,WHITE);
	}
};
class SquareAsteroid : public Asteroid {
public:
	SquareAsteroid(int w, int h) : Asteroid(w, h) { baseDamage = 10; bonus = 10; }
	void Draw() const override {
		Renderer::Instance().DrawPoly(transform.position, 4, GetRadius(), transform.rotation, WHITE);
	}
};
class PentagonAsteroid : public Asteroid {
public:
	PentagonAsteroid(int w, int h) : Asteroid(w, h) { baseDamage = 15; bonus = 15; }
	void Draw() const override {
		Renderer::Instance().DrawPoly(transform.position, 5, GetRadius(), transform.rotation,WHITE);
	}
};
class SpecialAsteroid : public Asteroid {
public:
	SpecialAsteroid(int w, int h) : Asteroid(w, h) { baseDamage = 0; bonus = 100; }
	void Draw() const override {
		Renderer::Instance().DrawUfo(transform.position, 4, GetRadius(), transform.rotation, PINK);
	}
};


// Shape selector
enum class AsteroidShape { TRIANGLE = 3, SQUARE = 4, PENTAGON = 5, SPECIAL = 6, RANDOM = 0 };
...
static inline std::unique_ptr<Asteroid> MakeAsteroid(int w, int h, AsteroidShape shape) {
	...
	case AsteroidShape::SPECIAL:
		return std::make_unique<SpecialAsteroid>(w, h);
	default: {
...
enum class WeaponType { LASER, BULLET, SPECIAL, COUNT };
class Projectile {
public:
	...
	void Draw() const {
		if (type == WeaponType::BULLET) {
			...
			float rotation = 180 * Utils::RandomFloat(0, 1);
			DrawPoly(transform.position, 3, 12,rotation, PINK);
			DrawPoly(transform.position,3,12,180+rotation, PINK);
...
protected:
	...
	float      fireRateSpecial;

	float      spacingLaser;
	float      spacingBullet;
	float      spacingSpecial;
			...
			for (auto pit = projectiles.begin(); pit != projectiles.end();) {
				...
				for (auto ait = asteroids.begin(); ait != asteroids.end(); ++ait) {
					...
					if (dist < (*pit).GetRadius() + (*ait)->GetRadius()) {
						player->AddScore(10);
						if (currentWeapon == WeaponType::SPECIAL) {
							player->AddScore(10);
